...
Welcome to Module 2: **Calculating Sample Size from Margin of Error**

In this module, we’ll explore how margin of error (MOE) arguments can guide sample size determination. This approach differs from power analysis, as it doesn’t involve hypothesis testing and therefore doesn’t require the concept of power. Instead, it relies solely on the concept of *precision*. Larger sample sizes lead to more precise estimates. This method is invaluable not only in Malaria Molecular Surveillance (MMS) but also in epidemiology more broadly, where estimating prevalence is often a primary goal.
