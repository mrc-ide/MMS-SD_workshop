...
$$
y = \frac{|p_1 - p_0|}{\sqrt{\frac{p_1(1-p_1)}{N}}}
$$
The vertical lines around $|p_1 - p_0|$ mean we should take the *absolute value* of the difference. This ensures that $y$ is always positive.
...
The following R code implements this sample size formula. Have a play around with this code. Try changing the value of `p1` and see what happens to the minimum sample size. Do you obtain the value $N=74$ when $p_1=0.3$, like we found from the power curve? Remember that you should always round sample sizes *up* if they are non-integers. What happens to the sample size as `p1` gets closer to 15%?
