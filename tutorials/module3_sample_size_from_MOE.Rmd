---
title: "Module 3: Calculating Sample Size from Margin of Error"
...
*Disclaimer: This scenario is entirely fictitious, and while it uses real place names, any resemblance to actual events is purely coincidental*
...
All three answers above are reasonable as long as they can be justified. But for the sake of argument let's assume a value of $p=0.26$ going forward. The NMCP has decided that a MOE of 5% is acceptable. Complete the following R code by entering the appropriate values for `p` and `m` to compute the resulting sample size:
