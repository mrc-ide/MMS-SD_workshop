...
You should see a sample size table with the number of sites (health facilities) in rows, and the assumed prevalence of *pfhrp2/3* deletions in columns (like below).
...
To balance these factors, we recommend a default assumed prevalence of 10%. This value provides a reasonable sensitivity level while keeping sample sizes manageable. For more insights on selecting this parameter, refer to the [FAQ section](https://shiny.dide.ic.ac.uk/DRpower-app/) in the web-based *pfhrp2/3* Planner.
...
quiz(caption = "QUIZ - Study design",
  question_checkbox("Which of these designs would you choose for this study?",
           allow_retry = TRUE,
           answer("Sentinel site surveillance ", correct = TRUE),
...
quiz(caption = "QUIZ - Probability above threshold",
  question_checkbox("Which of these is true?",
           allow_retry = TRUE,
           answer("There is *less than 95% probability* that the prevalence is above the 5% threshold. Therefore we conclude that the prevalence is below 5%.", correct = FALSE),
           answer("There is *more than 95% probability* that the prevalence is above the 5% threshold. Therefore we conclude that the prevalence is above 5%.", correct = TRUE)
  )
)
```
...
Still on the [Analysis tab](https://shiny.dide.ic.ac.uk/DRpower-app/), scroll down to the "Estimate ICC" button (it should look like the screenshot below). Clicking this button gives an estimate of the ICC, including a point estimate and a 95% CrI.

![](images/est_icc.png){width="100%"}

```{r quiz-icc}
quiz(caption = "QUIZ - Intra-cluster correlation",
  question_checkbox("Based on the ICC analysis, which of these is true:",
           allow_retry = TRUE,
